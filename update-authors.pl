...
sub ReplaceAuthors {
  ...
  $_ eq 'Martin Wimpress <code%flexion.org>' and $_ = 'Martin Wimpress <martin%nate-desktop.org>' for @authors;
  ...
  $_ eq 'raveit65 <chat-to-me%raveit.de>' and $_ = 'Wolfgang Ulbrich <nate%raveit.de>' for @authors;
  $_ eq 'raveit65 <nate%raveit.de>' and $_ = 'Wolfgang Ulbrich <nate%raveit.de>' for @authors;
  $_ eq 'rbuj <robert.buj%gmail.com>' and $_ = 'Robert Buj <robert.buj%gmail.com>' for @authors;
  $_ eq 'Scott Balneaves <sbalneav%ltsp.org>' and $_ = 'Scott Balneaves <sbalneav%nate-desktop.org>' for @authors;
  $_ eq 'spuhpointer <madars.vitolins%gmail.com>' and $_ = 'Madars Vitolins <madars.vitolins%gmail.com>' for @authors;
  $_ eq 'Wolfgang Ulbrich <chat-to-me%raveit.de>' and $_ = 'Wolfgang Ulbrich <nate%raveit.de>' for @authors;
  return @authors;
}
...
sub GetNewAuthors {
  my @authors = `git log --pretty="%an <%ae>" --since "2012-01-01" -- . "_.h" "_.c" | sort | uniq | sed 's/@/%/g' | sed '/^nate-i18n.*/d'`;
  chomp @authors;
  return ReplaceAuthors(@authors);
}
